...
EL FLOAT_Type = {sizeof(float), L"вещественные числа float", float_zero, float_input, float_add, float_mul, float_copy,
                 float_show
                };
EL DOUBLE_Type = {
  sizeof(double), L"вещественные числа double", double_zero, double_input, double_add, double_mul, double_copy,
  double_show
};

// Создание квадратной матрицы размера N * N заполненной нулями
Matrix matrix_create(EL T, int N) {
  Matrix m;
  m.TYPE = T;
...
Matrix matrix_add(Matrix a, Matrix b) {
  ...
  Matrix m = matrix_create(a.TYPE, a.N);
  // Суммируем в цикле все элементы
  for (int i = 0; i < m.N; i++) {
    for (int j = 0; j < m.N; j++) {
      m.data[i][j] = a.TYPE.add(a.data[i][j], b.data[i][j]);
    }
  }
  return m;
}
...
Matrix matrix_mul(Matrix a, Matrix b) {
  ...
  for (int i = 0; i < m.N; i++)
    for (int j = 0; j < m.N; j++)
      for (int k = 0; k < m.N; k++) {
        m.data[i][j] = a.TYPE.add(m.data[i][j], a.TYPE.mul(a.data[i][j], b.data[i][j]));
      }
  return m;
}

// Умножение матрицы на скаляр
// Если скаляр точно такого же типа как и элементы матрицы
Matrix matrix_scalar(Matrix a, void *scalar) {
  Matrix res = matrix_create(a.TYPE, a.N);  // Матрица-результат
  for (int i = 0; i < res.N; i++)           // Цикл по строкам
    for (int j = 0; j < res.N; j++)         // Цикл по столбцам
      res.data[i][j] = a.TYPE.mul(a.data[i][j], scalar);
  return res;
}
...
Matrix matrix_linear_transformation_1(Matrix a, int destination, int source, void *coef) {
  ...
  for (int i = 0; i < res.N; i++) {    // Цикл по строкам
    for (int j = 0; j < res.N; j++) {  // Цикл по столбцам
      res.data[i][j] = a.TYPE.copy(a.data[i][j]);
    }
  }
  for (int k = 0; k < res.N; k++) {
    // Цикл по столбцам
    // Строка[destination] = Строка[destination] + Строка[source] * coef
    res.data[destination - 1][k] = a.TYPE.add(res.data[destination - 1][k], a.TYPE.mul(res.data[source - 1][k], coef));
  }
  return res;
}
...
Matrix matrix_linear_transformation_2(Matrix a, int destination, int source, void *coef) {
  ...
  for (int k = 0; k < res.N; k++) {
    ...
    res.data[k][destination - 1] = a.TYPE.add(res.data[k][destination - 1], a.TYPE.mul(res.data[k][source - 1], coef));
  }
  return res;
}
