...
int menu(const wchar_t *title, int N, ...)
{
    ...
    wprintf(L"\n%ls:\n", title);  // Печатаем заголовок меню
    // Печатаем меню
    va_list args;  // Обьявляем что забирать параметры из ...  через переменную args
    va_start(args, N);                              //  N параметров
    for (int i = 1; i <= N; i++)
    {                  // i=1,2,3...N - номер пункта меню
...
{
    wprintf(L"Умножение на скаляр\n");
    wprintf(L"Ввод матрицы A\n");
    Matrix a = matrix_input(TYPE);
    wprintf(L"Ввод числа:\n");
...
int main() {
    ...
    while (1) {
        int op = menu(L"Возможные операции (0-выход)", 6, L"Матричное сложение", L"Матричное умножение",
                      L"Умножение на скаляр", L"Прибавление к строке линейной комбинации других строк", L"Прибавление к столбцу линейной комбинации других столбцов", L"Транспонирование");
        switch (op) {
            case 1:
